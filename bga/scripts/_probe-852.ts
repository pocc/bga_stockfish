...
async function main() {
  ...
  try {
    ...
    await page.goto('https://boardgamearena.com/table?table=852862107', { waitUntil: 'domcontentloaded' }).catch(() => {});
    await page.waitForTimeout(2000);
    const cookies = await ctx.cookies();
    const idt = cookies.find(c => c.name === 'TournoiEnLigneidt')?.value;
    const url = 'https://boardgamearena.com/table/table/tableinfos.html?id=852862107&nosuggest=true&table=852862107&noerrortracking=true&dojo.preventCache=' + Date.now();
